...
def test_streak_match_dedupes_same_day_using_updated_at_utc_ms() -> None:
    quest = SimpleNamespace(
        ...
        current_progress=5,
        required_progress=30,
        updated_at_utc_ms=int(datetime(2026, 2, 22, 10, tzinfo=timezone.utc).timestamp() * 1000),
        created_at=datetime(2026, 2, 20, tzinfo=timezone.utc),
    )
...
def test_one_time_keyword_and_activity_matching() -> None:
    quest = SimpleNamespace(
        ...
        template=SimpleNamespace(parameters='{"keywords":["5K","5 km"],"activity":"running"}'),
    )
    entry = _make_entry(datetime(2026, 2, 22, tzinfo=timezone.utc), "I completed my first 5k run today")
...
def test_recursive_respects_progress_cap_and_same_day_dedupe() -> None:
    ...
    quest_same_day = SimpleNamespace(
        ...
        current_progress=1,
        required_progress=3,
        updated_at_utc_ms=int(datetime(2026, 2, 22, 8, tzinfo=timezone.utc).timestamp() * 1000),
        created_at=datetime(2026, 2, 20, tzinfo=timezone.utc),
    )
    entry = _make_entry(datetime(2026, 2, 22, 18, tzinfo=timezone.utc))

    assert check_quest_match(quest_done, entry, ["Meditation"], ["meditated"]) is False
    assert check_quest_match(quest_same_day, entry, ["Meditation"], ["meditated"]) is False
