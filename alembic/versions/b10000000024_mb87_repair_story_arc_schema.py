"""MB87 — Repair legacy story arc schemas after Week 7 rollout.

Revision ID: b10000000024
Revises: b10000000023
Create Date: 2026-03-14 14:00:00.000000

Some local/dev SQLite databases were stamped to MB84+ while still carrying the
older ``story_arcs`` / ``arc_triggers`` layouts. This migration repairs those
tables in place without dropping user data, and is a no-op when the schema is
already canonical.
"""
...
revision: str = "b10000000024"
down_revision: Union[str, Sequence[str], None] = "b10000000023"
...
def _now_fixed_ms() -> str:
    return _normalize_timestamp(datetime.now(UTC)) or "1970-01-01T00:00:00.000Z"
...
def _ensure_story_arcs_indexes(bind: sa.engine.Connection) -> None:
    ...
    elif "idx_story_arcs_active_unique" not in existing:
        op.create_index(
...
def _repair_sqlite(bind: sa.engine.Connection) -> None:
    repair_now = _now_fixed_ms()
    story_rows = _load_rows(bind, "story_arcs") if _table_exists(bind, "story_arcs") else []
    trigger_rows = _load_rows(bind, "arc_triggers") if _table_exists(bind, "arc_triggers") else []

    bind.execute(sa.text("PRAGMA foreign_keys=OFF"))

    if _table_exists(bind, "arc_triggers"):
        bind.execute(sa.text("ALTER TABLE arc_triggers RENAME TO arc_triggers__legacy_mb87"))
    if _table_exists(bind, "story_arcs"):
        bind.execute(sa.text("ALTER TABLE story_arcs RENAME TO story_arcs__legacy_mb87"))

    _create_story_arcs_table()

    transformed_story_rows: list[dict[str, object]] = []
    repair_triggers: list[dict[str, object]] = []
    ...
    for row in story_rows:
        ...
        completed_at = _normalize_timestamp(row.get("completed_at") or row.get("ended_at"), None)
    ...
    active_by_user: dict[str, list[dict[str, object]]] = {}
    for row in transformed_story_rows:
        if row["status"] == "active":
            active_by_user.setdefault(str(row["user_id"]), []).append(row)
            ...
            repair_triggers.append(
                {
                    ...
                    "trigger_data": json.dumps(
                        {
                            "repair": "mb87_multiple_active_arcs",
    ...
    for row in trigger_rows:
        ...
        user_id = str(row.get("user_id") or "")
        if not arc_id or not user_id or arc_id not in valid_arc_ids:
            continue
    ...
    transformed_trigger_rows.extend(repair_triggers)
    ...
    if _table_exists(bind, "arc_triggers__legacy_mb87"):
        bind.execute(sa.text("DROP TABLE arc_triggers__legacy_mb87"))
    if _table_exists(bind, "story_arcs__legacy_mb87"):
        bind.execute(sa.text("DROP TABLE story_arcs__legacy_mb87"))
...
def upgrade() -> None:
    bind = op.get_bind()
    story_exists = _table_exists(bind, "story_arcs")
    trigger_exists = _table_exists(bind, "arc_triggers")
    story_ok = story_exists and _STORY_ARCS_COLUMNS.issubset(_columns(bind, "story_arcs"))
    trigger_ok = trigger_exists and _ARC_TRIGGERS_COLUMNS.issubset(_columns(bind, "arc_triggers"))

    if bind.dialect.name == "sqlite" and ((story_exists and not story_ok) or (trigger_exists and not trigger_ok)):
        _repair_sqlite(bind)
        return
...
def downgrade() -> None:
    """Irreversible data-preserving repair migration."""
    return None
